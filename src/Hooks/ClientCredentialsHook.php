...
class ClientCredentialsHook implements AfterErrorHook, BeforeRequestHook, SDKInitHook
{
    ...
    private function getCredentialsGlobal(\Closure $securitySource): ?Credentials
    {
        ...
        if (! $security instanceof Components\Security) {
            throw new \InvalidArgumentException('Invalid security type: expected Components\Security, got '.get_class($security));
        }
    ...
    private function doTokenRequest(string $baseUrl, Credentials $credentials, array $scopes): Session
    {
        ...
        try {
            $tokenResponse = $serializer->deserialize($responseBody, 'Dwolla\Hooks\TokenResponse', 'json', DeserializationContext::create()->setRequireAllRequiredProperties(true));
        } catch (\Exception $ex) {
